                    ...
                    @foreach ($row as $slot)
                        <td class="border border-gray-300 p-4 w-[200px] h-[150px] bg-gray-100 align-middle text-center city-cell"
                            ...
                            >
                                ...
                                {{-- Event effects from event type --}}
                                    @if ($slot->event_id && $slot->event)
                                        <div class="event-effect-data hidden">
                                            @php
                                                $effects = optional($slot->event->eventType)->effects ?? [];
                                            @endphp
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        document.querySelector('tbody').addEventListener('mouseover', async (event) => {
            ...
            // Get event effects from data attributes
            const eventEffects = {};
            if (slot.dataset.eventId) {
                const eventEffectElements = slot.querySelectorAll('.effect-event');
            ...
            // Get module effects from adjacent cells
            const moduleEffects = {};
            for (let r = row - 1; r <= row + 1; r++) {
                for (let c = col - 1; c <= col + 1; c++) {
                    if (r < 0 || r >= rows || c < 0 || c >= cols) continue;
                    ...
                    if (moduleSlot) {
                        const effectElements = moduleSlot.querySelectorAll('.effect');
