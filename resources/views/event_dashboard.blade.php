        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    @foreach($activeEvents as $slotId => $event)
                        @if(!str_contains($event['event_name'], '(Aangrenzend)'))
                            ...
                            {{ $running ? 'Nog' : 'Start over' }} {{ $event['time_remaining'] }}
                                    ...
                                    @if(!$running)
                                        <span
                                            class="ml-2 bg-gray-200 text-gray-800 px-2 py-1 rounded text-xs">Gepland</span>
                                    @endif
                                </div>

                                <form
                                    action="{{ route('events.reset') }}?time={{ request()->query('time', date('H:i:s')) }}"
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 lg:p-8">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-6">Selecteer Event voor
                        Specifiek Vakje</h3>

                    <form action="{{ route('events.set') }}?time={{ request()->query('time') }}" method="POST"
                          id="eventForm">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <input type="text" name="event_description" id="event_description"
                                       value="{{ old('event_description') }}" required
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100"
                        ...
                        <div
                            class="mb-6 p-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
